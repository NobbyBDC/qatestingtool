...
export default function ZapSecurity() {
  ...
  const runScan = async (e) => {
    ...
    setResults(null);
    try {
      const { data } = await api.post('/tests/zap', { url, apiKey: apiKey || undefined }, { timeout: 300000 });
      setResults(data);
    } catch (err) {
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 mb-6">
        <form onSubmit={runScan} className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {loading && <span className="text-xs text-slate-500">Active scan may take 2–5 minutes</span>}
          </div>
        </form>
      </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <p className="text-xs text-slate-500 font-mono">{results.scanId}</p>
                  {!results.real && (
                    <span className="text-xs text-yellow-500">(demo — ZAP daemon not running)</span>
                  )}
                </div>
              </div>
